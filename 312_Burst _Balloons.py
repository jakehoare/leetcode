...
# all possible balloons to be burst last.  Coins gained from bursting a balloon last is coins from that balloon *
# cons from balloon before the subarray * coins from balloon after the subarray + coins from left subarray + coins
# from right subarray.
# Time - O(n**3)
# Space - O(n**2)
...
class Solution(object):
    def maxCoins(self, nums):
        ...
        for length in range(1, n+1):
            ...
            for left in range(1, n+2-length):   # consider subarray from left to right inclusive
                right = left+length-1

                for last in range(left, right+1):
                    this_coins = nums[left-1] * nums[last] * nums[right+1]  # this balloon time those outside subarray
