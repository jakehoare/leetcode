...
# Iterate over seq. A closing bracket causes the depth to decrease, an opening bracket increases depth.
# When depth is even, the bracket is part of sunbsequence A (result is 0), else part of B (result is 1).
# Update result after processing closing brackets and before processing opening brackets.
# Time - O(n)
# Space - O(1)
...
class Solution(object):
    def maxDepthAfterSplit(self, seq):
        ...
        for c in seq:
            if c == ")":        # decrement before c
                depth -= 1
            result.append(depth % 2)
            if c == "(":        # increment after c
                depth += 1
